...
export function AppShell({
  ...
  return (
    <div className="min-h-screen flex w-full">
        ...
        <header className="h-16 shrink-0 border-b border-border bg-background/40 backdrop-blur-xl sticky top-0 z-30">
          <div className="h-full px-4 md:px-8 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <Link to="/" className="md:hidden h-9 w-9 rounded-xl bg-gradient-to-br from-primary to-primary/40 grid place-items-center shrink-0" aria-label="Home">
                <Power className="h-4 w-4 text-primary-foreground" strokeWidth={2.5} />
              </Link>
              <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 md:gap-3 shrink-0">
              <form onSubmit={runSearch} className="hidden lg:flex items-center gap-2 glass rounded-lg px-3 py-1.5 w-64 focus-within:border-primary/40">
                ...
                />
                <kbd className="text-[10px] text-muted-foreground bg-muted/50 px-1.5 py-0.5 rounded">↵</kbd>
              </form>
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm grid place-items-start pt-24 px-4" onClick={() => setSearchOpen(false)}>
          <form onSubmit={runSearch} onClick={(e) => e.stopPropagation()} className="w-full max-w-md glass rounded-2xl p-2 flex items-center gap-2">
            ...
            />
            <button type="button" onClick={() => setSearchOpen(false)} className="h-8 w-8 grid place-items-center text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
          </form>
        </div>
